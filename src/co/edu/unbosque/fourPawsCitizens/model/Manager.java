...
//                    try {
//                    	
//                    	String estaMadre = delimitar.next();
//                    	if(estaMadre.matches("[0-9]*") ) {
//                    		System.out.println(estaMadre +"			Es está guardando    "+ a);
//                    		a++;
//                    		  e.setMicrochip(Long.parseLong(estaMadre));
//                              System.out.println(e.getMicrochip());
//                              e.setId("NO-ID");
...
//                              } else {
//                                  e.setPotentiallyDangerous(true);
//                                  if(delimitar.next().isEmpty()==false)	{
...
//
//                        }else {
//                        	System.out.println("Pasa por el error del microchips");
//                        }
//                    } catch (EmptyAttributeException s) {
//                    	   System.out.println("lllllllllllllllllllllllllllll");
//                    } catch (NumberFormatException s) {
...
//			petArray.get(i).setId(microChip + "-" + species + sex + size + potencialDanger + "-" + neighborhood);
	...
	public String mostrarPet(Pet pet) {
		String contenido = "";
		contenido = contenido + pet.getId() + ";" + pet.getMicrochip() + ";" + pet.getSpecies() + ";" + pet.getSex()
				+ ";" + pet.getSize() + ";" + pet.getPotentiallyDangerous() + ";" + pet.getNeighborhood() + "\n";
		return contenido;
	}
		...
		for (int i = 0; i < petArray.size(); i++) {
			if (num == petArray.get(i).getMicrochip()) {
				contenido = "ID: " + petArray.get(i).getId() + "\n" + "Species: " + petArray.get(i).getMicrochip()
						...
						+ petArray.get(i).getPotentiallyDangerous() + "\n" + "Neighborhood: "
						+ petArray.get(i).getNeighborhood() + "\n";
			}
		}
	...
	public String findBypotentDangerousInNeighborhood(int n, String range, String neighborhood) {
		...
		if (range.equals("TOP")) {
			for (int i = 0; i < petArray.size(); i++) {
				if (petArray.get(i).getPotentiallyDangerous() == true
						&& petArray.get(i).getNeighborhood().equals(neighborhood)) {
